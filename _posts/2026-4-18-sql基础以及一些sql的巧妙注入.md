...
$stmt = $pdo->prepare($sql); //prepare是mysqli类的一个方法，专门用来准备sql预编译语句
$stmt->execute([$username, $password]);
$result = $stmt->fetch();
```

先是定义一个sql语句，再定义语句里待填的参数，最后赋值进语句里查询，避免了sql拼接造成的注入。当然不同语言写法不尽相同。

但并不是什么地方都能用预编译写法，预编译写法使用的是参数赋值的手段。不带具体数值的sql语句就不能使用预编译的手段了
...
预编译也并非绝对安全，历史上存在溢出而造成sql注入的漏洞CVE-2024-27304 
